...
def getRootMeanSquareError(M,validationOutput,phi,lambdaval,w,size):
    ...
    for i in range(0,len(validationOutput)):
        ...
        for j in range(1,M):
            t+=w[j]*phi.item(i,j)
        print(i)
        error+= (validationOutput[0][i+16000]-t)**2
        error+=lambdaval*matmul(w.transpose(),w)
        rms=sqrt(error/len(validationOutput))
    return rms

def trainClosedForm(data,outputLabels):
    trainingData=data[0:16000]
    validationData=data[16000:18000]
    testData=data[16000:20000]

    trainingOutput=outputLabels[0:16000]
    validationOutput=outputLabels[16000:18000]
    testOutput=outputLabels[18000:20000]
    size=len(data)
    ...
    for M in range(2,3):
        ...
        while(lambdaval<10):
            ...
            for i in range(0,len(validationOutput)):
                ...
                error+= (validationOutput[0][i+int(16000)]-t)**2
                error+=lambdaval*matmul(w.transpose(),w)
